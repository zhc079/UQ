...
E_000 = np.loadtxt("votcaMADN000/E_qmmm.txt")
E_005 = np.loadtxt("votcaMADN005/E_qmmm.txt")
E_010 = np.loadtxt("votcaMADN010/E_qmmm.txt")
E_015 = np.loadtxt("votcaMADN015/E_qmmm.txt")
E_020 = np.loadtxt("votcaMADN020/E_qmmm.txt")
E_025 = np.loadtxt("votcaMADN025/E_qmmm.txt")
...
for fi in range(2):
    for fj in range(3):
        ...
        ax00.set_xlabel(r"$E(\alpha_{HFX}=0.25)$[eV]",color="black", fontsize=tick_font_size)
        ax00.set_ylabel(r"$E$"+str1+"[eV]",color="black",fontsize=tick_font_size)
...
sigma_Eqmmm = [0.0774, 0.0726,0.0773,0.0867,0.0803,0.0841]
gs00 = gridspec.GridSpecFromSubplotSpec(2, 2,  width_ratios=(4, 1), height_ratios=(1, 4),
                            wspace=0.01, hspace=0.01, subplot_spec=gs0[fi,fj])
# Create the Axes.
ax00 = fig.add_subplot(gs00[1, 0])
ax00.plot(HFX, sigma_Eqmmm, "k--o", markersize=8)
ax00.set_facecolor('white')
ax00.xaxis.set_ticks(np.arange(0.0, 0.3, 0.1))
ax00.yaxis.set_ticks(np.arange(0.07, 0.10, 0.01))
...
ax00.set_ylabel(r"$\sigma(E)$ [eV] ", color="black",fontsize=tick_font_size)

plt.setp(ax00.spines.values(), color="black")
ax00.axis("on")

plt.tight_layout()
plt.savefig("scatterE_qmmm.pdf",dpi=400)
...
for i in range(1,7):
    print("--IFNO: energy range: ",i, np.max(dict_E[int(i)]), np.min(dict_E[int(i)]))