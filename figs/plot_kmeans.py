...
plt.tick_params(
    ...
    labelbottom=False) 
fig.savefig("fig_distri_eigenVecRW.pdf",dpi=600)
...
for i in np.arange(1,11,1):
    ...
    print('--info: cost is:',cost)
    all_cost_3c_RW.append(cost)

############### for L_W 
x1 = np.loadtxt("PBE0/eigenvec1Lw.txt")
x2 = np.loadtxt("PBE/eigenvec1Lw.txt")
x3 = np.loadtxt("B3LYP/eigenvec1Lw.txt")
x4 = np.loadtxt("BHANDHLYP/eigenvec1Lw.txt")
x5 = np.loadtxt("TPSS/eigenvec1Lw.txt")
x6 = np.loadtxt("BP86/eigenvec1Lw.txt")
x7 = np.loadtxt("wB97X/eigenvec1Lw.txt")
x8 = np.loadtxt("wB97X-D3/eigenvec1Lw.txt")
x9 = np.loadtxt("M06L/eigenvec1Lw.txt")
x10 = np.loadtxt("BHLYP/eigenvec1Lw.txt")
ToF = [1.23e-3,1.04e-3,4.28e-3,737.94,1.37e-2,7.46e-3,2.92e-2,0.0489,0.319,2.47]
dict_eigVec = {1:x1,2:x2,3:x3,4:x4,5:x5,6:x6,7:x7,8:x8,9:x9,10:x10}
...
plt.tick_params(
    ...
    top=False,         # ticks along the top edge are off
    labelbottom=False) 
fig.savefig("fig_distri_eigenVecL.pdf",dpi=600)
...
for i in np.arange(1,11,1):
    ...
    for ii in range(cluster2.shape[0]):
        cost += (data[cluster2[ii],0] - center2 )**2
    print('--info: cost is:',cost)
    all_cost_3c_LW.append(cost)

fig, (ax1, ax2) = plt.subplots(1, 2,figsize=(10, 5))
ax1.scatter(ToF,all_cost_2c_LW,edgecolor="black",facecolor="None", label=r"$K=2$")
ax1.scatter(ToF, all_cost_3c_LW,color="red",s=50,label=r"$K=3$")
...
ax1.annotate("(a)", xy=(-0.21,1), xycoords='axes fraction', size=20)
ax1.annotate("Normal Laplacian", xy=(0.15,1.05), xycoords='axes fraction', size=20)

ax2.scatter(ToF,all_cost_2c_RW,edgecolor="black",facecolor="None", s=50,label=r"$K=2$")
ax2.scatter(ToF, all_cost_3c_RW,color="red",s=50,label=r"$K=3$")
...
ax2.legend(fontsize=16,frameon=True,loc=3)
ax2.annotate("(b)", xy=(-0.21,1), xycoords='axes fraction', size=20)
ax2.annotate("Random-walk Laplacian", xy=(0.1,1.05), xycoords='axes fraction', size=20)

plt.tight_layout()
fig.savefig("fig_Z_ToF.pdf",dpi=400)