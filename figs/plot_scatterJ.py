...
def scatter_hist2(x, y, ax, ax_histx, ax_histy):
    # no labels
...
for fi in range(3):
    for fj in range(3):
        ...
        str2 = dict_Jstr[2+fi*3+fj]
        ax00.set_xlabel(r"log$_{10}J^2$(PBE0)",fontsize=10)
        ax00.set_ylabel(r"log$_{10}J^2$"+str2,fontsize=10)
        ...
        scatter_hist2(np.log10(dict_J[1]**2), np.log10(dict_J[2+fi*3+fj]**2), ax00, ax00_histx, ax00_histy)
        #scatter_hist2(np.log10(dict_J[1]**2), np.log10(dict_E[2+fi*3+fj]**2), ax00, ax00_histx, ax00_histy)

plt.tight_layout()
plt.savefig("scatterJ_all.pdf",dpi=500)