...
\Zhong{}{It is worth noting that in multiscale modeling there is usually correlations between the site energies $E_i$. 
Those correlations are due to electrostatic interactions, while the distribution $P(\log_{10}(\tau))$ evaluated from the Monte Carlo sampling does not take the energy correlations into account.
However, in the case of MADN, these correlations are notably absent. This can be attributed to the specific molecular arrangement and electronic structure of MADN, which effectively decouples the site energies from one another. As a result, the site energies in MADN are practically uncorrelated, simplifying the analysis of their contributions to the time-of-flight distribution. }
...
We perform such mobility calculations for the six explicit \ahfx models as well as the Monte-Carlo sampled uncertainty for six different values of $F$. The results are shown in Fig.~\ref{fig:fig_mle_withE_mu2_ave}. While the distributions shift depending on the strength of the applied field, the overall shape and the width of the confidence intervals is very similar. The width of the confidence intervals in $\log_{10}(\mu)$ decreases from \unit[0.9]{} at low fields ($\unit[4 \cdot 10^7]{V/m}$) to \unit[0.7]{} at a field of $\unit[9 \cdot 10^7]{V/m}$, corresponding broadly to a width of close to one order of magnitude in $\mu$. A stronger drift component due to a larger external electric field seems to make the mobility slightly less sensitive to uncertainties \bjoern{CHECK}{not sure about this interpretation}.

\Zhong{}{The decrease in the confidence intervals of $\log_{10}(\mu)$ indicates a reduction in the relative uncertainty in charge mobility, so a higher fields lead to a more deterministic mobility values.
There are some possible physical reasons for such reduction in uncertainty. At low electric fields, the charge carriers spend a significant amount of time in some low-energy sites which are generated in the Monte Carlo sampling. 
As the field strength increases, the drift term becomes more dominant, reducing the impact of these stochastic factors. 

Furthermore, a stronger electric field helps the carrier to escape from the low energy sites more efficiently. This effect would reduce variability in observed mobilities, contributing to a narrowing confidence interval.
So a stronger applied field, which is deterministic, results in a more predictable mobility response, therefore reducing the spread in $\log_{10}(\mu)$ distribution.
}
