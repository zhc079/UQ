...
The equation of motion for updating the atomic coordinates is implemented by leap-frog algorithm~\cite{van_gunsteren_leap-frog_1988} with a time step of \unit[1]{fs}. 
...
Figure \ref{fig:E_qmmm_MADN} presents a scatter plot comparing the site energies of MADN molecules across different HFX values, using $\alpha$ = 0.25 (PBE0 functional) as a reference for which the calculated molecule energy has a range from -1.12 to -0.63 \unit[]{eV}. The range of molecule energy for the other five HFX are: from -1.17 to -0.60 \unit[]{eV} for $\alpha=0$, from -1.11 to -0.59 \unit[]{eV} for $\alpha=0.05$, from -1.07 to -0.63 \unit[]{eV} for $\alpha=0.10$, from -1.13 to -0.61 \unit[]{eV} for $\alpha=0.15$, and from -1.16 to -0.60 \unit[]{eV} for $\alpha=0.20$.
...
\begin{equation}
    W_1 (P_{E(\alpha=0)}, P_{E(\alpha=0.25)}) = (\frac{1}{N} \sum\limits_{i=1}^N | E_{(i)}(\alpha=0) - E_{(i)}(\alpha=0.25) |)
\end{equation}
...
\begin{table}[H]
 ...
 0.20 & 0.12 & -1.70 & -0.82 \\
    \hline
    \end{tabular}
    \caption{ In the tested $\alpha'$ values, the second column shows the maximum of $E_i(\alpha=0.25) - E_i(\alpha')$ for all molecules. Denoting the index of the molecule which has maximum $E_i(\alpha=0.25) - E_i(\alpha')$ for the chosen $\alpha'$ case, the third column shows the energy of molecule $i'$ calculated by $\alpha=0.25$. The fourth column shows the molecule energy $E_{i'}(\alpha')$ calculated by $\alpha'$. All the energies are in unit \unit[]{eV}.}
    \label{tab:maxEi}
\end{table}
%
Table \ref{tab:maxEi} shows that depending on $\alpha'$, the maximum $|E_i(\alpha=0.25) - E_i(\alpha')|$ is 0.12 to 0.15, which is considerable small compared to the range of molecular energy. 
